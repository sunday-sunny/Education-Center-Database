...
create or replace function fnLogin_ST(
    pname varchar2,
    ppw varchar2
) return varchar2
is 
    vseq tblstudent.student_seq%type;
begin

    select student_seq into vseq from tblstudent where name = pname and idcard_number = ppw;   
    return vseq;
...
from tblstudent where name = '김조하' and idcard_number = '1770114';
--S0038번 교육생으로 로그인한다.



-- *** 이름과 패스워드를 넣으면 로그인 성공 여부 보여주는 프로시저 ***
--a)
create or replace procedure procLoginCheck_ST(
    pname varchar2,
    ppw varchar2
)
is
    vcnt number;
begin
    select count(*) into vcnt from tblstudent where name = pname and idcard_number = ppw;
...
end procLoginCheck_ST;


execute procLoginCheck_ST('김조하', '1770114');
...
declare
     vresult number;
begin
    procLoginCheck_ST('김조하', '1770114', vresult);
...
-- *** 개설과정 별 개설과목 목록을 보여주는 뷰 ***    
create or replace view vw_openCourseAndSub_ST
as
select
...
select * from vw_opencourseandsub_st;
...
-- *** 과정 기간이 끝나지 않은 또는 중도 탈락 처리된 교육생의 목록 뷰 ***
create or replace view vwfailed_ST
as
select * from tblregister
where completion_status = 'N' or dropout_date is not null;

select * from vwfailed_st;
...
where (r.completion_status = 'N' or r.dropout_date is not null)
    and r.student_seq = (select student_seq from vwfailed_st where open_course_seq = 'OL001')
